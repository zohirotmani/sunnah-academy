...
function openBookInline(event,id){
  ...
  openBookInlineContent(id);

  return false;
}

async function openBookInlineContent(id){
  const reader=document.getElementById("inlineBookReader");
  ...
  reader.innerHTML=`
    <div class="inline-book-head">
      <div>
        <span class="eyebrow">قراءة الكتاب</span>
        <h3>جاري تحميل الكتاب...</h3>
      </div>
      <button class="btn small ghost" type="button" onclick="closeInlineBook()">إغلاق</button>
    </div>
    <div class="inline-book-loading">جاري التحميل...</div>
  `;
    ...
    reader.innerHTML=`
      <div class="inline-book-head">
        <div>
          <span class="eyebrow">قراءة الكتاب</span>
          <h3>${esc(b.title)}</h3>
          <p>${esc(b.scholar_name||b.author||"")}</p>
        </div>

        <button
          class="btn small ghost"
          type="button"
          onclick="closeInlineBook()">
          إغلاق
        </button>
      </div>

      <div class="reader-actions inline-reader-actions">
        <button
          id="inlineSaveProgress"
          class="btn primary"
          type="button">
          حفظ تقدمي
        </button>
      ...
      <div class="pdf-frame inline-pdf-frame">
        <iframe
          src="${esc(b.pdf_path)}"
          title="${esc(b.title)}">
        </iframe>
...
async function loadLibrary(){const g=document.getElementById("libraryGrid"),s=document.getElementById("bookSearch");if(!g)return;try{const b=await getJSON("/api/books");const render=a=>g.innerHTML=a.length?a.map(bookCard).join(""):`<div class="empty">لا توجد نتائج.</div>`;render(b);if(s)s.addEventListener("input",()=>{const q=s.value.trim().toLowerCase();render(b.filter(x=>`${x.title} ${x.author||""} ${x.scholar_name||""}`.toLowerCase().includes(q)))})}catch{g.innerHTML=`<div class="empty">تعذر تحميل المكتبة.</div>`}}
async function loadBookPage(){const i=document.getElementById("bookInfo"),p=document.getElementById("pdfBox");if(!i||!p)return;const id=new URLSearchParams(location.search).get("id");try{const b=(await getJSON("/api/books")).find(x=>String(x.id)===String(id));if(!b){i.innerHTML="<div class='empty'>الكتاب غير موجود.</div>";return}i.innerHTML=`<div class="book-cover"><span>${esc(b.title)}</span></div><div><span class="eyebrow">كتاب</span><h1>${esc(b.title)}</h1><p>${esc(b.description||"")}</p><div class="book-meta">${b.scholar_name?`<span class="badge">${esc(b.scholar_name)}</span>`:""}${b.level_name?`<span class="badge">${esc(b.level_name)}</span>`:""}${b.subject_name?`<span class="badge">${esc(b.subject_name)}</span>`:""}</div></div>`;p.innerHTML=`<iframe src="${esc(b.pdf_path)}" title="${esc(b.title)}"></iframe>`;const pg=document.getElementById("pageNumber");const sp=document.getElementById("saveProgress");if(sp){sp.addEventListener("click",async()=>{const page=Math.max(1,Number(pg.value)||1);const r=await fetch("/api/student/progress",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({book_id:b.id,last_page:page})});if(r.status===401){location.href="/login.html";return}sp.textContent=r.ok?"تم حفظ التقدم":"تعذر الحفظ"})}}catch{ i.innerHTML="<div class='empty'>تعذر تحميل الكتاب.</div>"}}
async function loadScholars(){const g=document.getElementById("scholarsGrid");if(!g)return;try{const s=await getJSON("/api/scholars");g.innerHTML=s.length?s.map(scholarCard).join(""):`<div class="empty">لا يوجد علماء مضافون.</div>`}catch{g.innerHTML=`<div class="empty">تعذر التحميل.</div>`}}
async function loadScholarPage(){const i=document.getElementById("scholarInfo"),g=document.getElementById("scholarBooks");if(!i||!g)return;const id=new URLSearchParams(location.search).get("id");try{const [s,b]=await Promise.all([getJSON("/api/scholars"),getJSON("/api/books")]);const x=s.find(z=>String(z.id)===String(id));if(!x){i.innerHTML="<h1>العالم غير موجود</h1>";return}i.innerHTML=`${scholarAvatar(x)}<span class="eyebrow">العالم</span><h1>${esc(x.name)}</h1><p>${esc(x.biography||"")}</p>`;const m=b.filter(z=>String(z.scholar_id)===String(id));g.innerHTML=m.length?m.map(bookCard).join(""):`<div class="empty">لا توجد كتب مضافة.</div>`}catch{g.innerHTML=`<div class="empty">تعذر التحميل.</div>`}}

document.addEventListener("DOMContentLoaded",()=>{loadHome();loadLevels();loadLevelPage();loadLibrary();loadBookPage();loadScholars();loadScholarPage()});
